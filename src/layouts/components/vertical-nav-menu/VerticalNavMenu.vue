      ...
      <div @mouseenter="mouseEnter" @mouseleave="mouseLeave">
        ...
        <component :is="scrollbarTag" ref="verticalNavMenuPs" class="scroll-area-v-nav-menu pt-2" :settings="settings" @ps-scroll-y="psSectionScroll" @scroll="psSectionScroll" :key="$vs.rtl">
          <template v-for="(item, index) in menuItemsUpdated">
            ...
            <!-- Group Header -->
            <span v-if="item.header && !verticalNavMenuItemsMin && role==item.role" class="navigation-header truncate" :key="`header-${index}`">
              {{ item.header }}
            </span>
            <!-- /Group Header -->

            <template v-else-if="!item.header  && role==item.role">
...
export default {
  ...
  mounted () {
    this.role=this.$store.state.auth.me.role
    this.setVerticalNavMenuWidth()
  }
}
